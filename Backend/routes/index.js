...
router.post('/documents/upload', authenticateToken, upload.single('file'), documentController.uploadDocument);
...
router.get('/admin/users-stats', authenticateToken, requireRole('admin'), documentController.getUsersWithStats);

// Admin: Get documents by user ID
router.get('/admin/users/:userId/documents', authenticateToken, requireRole('admin'), documentController.getDocumentsByUser);

// ==================== Chat Routes (Protected) ====================

// Send message (authenticated users, with permission check)
router.post('/chat/message', authenticateToken, chatController.sendMessage);

// Get chat history (with permission check)
router.get('/chat/history/:documentId', authenticateToken, chatController.getChatHistory);

// Clear chat history (with permission check)
router.delete('/chat/history/:documentId', authenticateToken, chatController.clearChatHistory);

// Admin: Get chat history by user ID
router.get('/admin/users/:userId/chats', authenticateToken, requireRole('admin'), chatController.getChatHistoryByUser);
...
router.get('/health', (req, res) => {
  res.json({ 
    status: 'ok', 
    timestamp: new Date().toISOString(),
    service: 'RAG Chatbot API with Authentication'
  });
});
