-- Enable pgvector extension
CREATE EXTENSION IF NOT EXISTS vector;

-- Documents table with embeddings
CREATE TABLE IF NOT EXISTS documents (
  id BIGSERIAL PRIMARY KEY,
  content TEXT NOT NULL,
  metadata JSONB,
  embedding VECTOR(768),
  created_at TIMESTAMP DEFAULT NOW()
);
Select * From documents;
Select * FROM document_metadata;
SELECT * From chat_history;
-- Document metadata table
CREATE TABLE IF NOT EXISTS document_metadata (
  id TEXT PRIMARY KEY,
  title TEXT NOT NULL,
  url TEXT,
  file_type TEXT,
  file_size INTEGER,
  created_at TIMESTAMP DEFAULT NOW(),
  schema TEXT
);

-- Document rows for structured data
CREATE TABLE IF NOT EXISTS document_rows (
  id SERIAL PRIMARY KEY,
  dataset_id TEXT REFERENCES document_metadata(id) ON DELETE CASCADE,
  row_data JSONB,
  created_at TIMESTAMP DEFAULT NOW()
);

-- Chat history table
CREATE TABLE IF NOT EXISTS chat_history (
  id BIGSERIAL PRIMARY KEY,
  document_id TEXT REFERENCES document_metadata(id) ON DELETE CASCADE,
  user_message TEXT NOT NULL,
  bot_response TEXT NOT NULL,
  created_at TIMESTAMP DEFAULT NOW()
);

-- Function to match documents by embedding similarity
CREATE OR REPLACE FUNCTION match_documents (
  query_embedding VECTOR(768),
  match_count INT DEFAULT 5,
  filter JSONB DEFAULT '{}'
) RETURNS TABLE (
  id BIGINT,
  content TEXT,
  metadata JSONB,
  similarity FLOAT
)
LANGUAGE plpgsql
AS $$
#variable_conflict use_column
BEGIN
  RETURN QUERY
  SELECT
    documents.id,
    documents.content,
    documents.metadata,
    1 - (documents.embedding <=> query_embedding) AS similarity
  FROM documents
  WHERE documents.metadata @> filter
  ORDER BY documents.embedding <=> query_embedding
  LIMIT match_count;
END;
$$;

-- Indexes for performance
CREATE INDEX IF NOT EXISTS idx_documents_embedding ON documents USING ivfflat (embedding vector_cosine_ops);
CREATE INDEX IF NOT EXISTS idx_documents_metadata ON documents USING gin (metadata);
CREATE INDEX IF NOT EXISTS idx_document_rows_dataset ON document_rows(dataset_id);
CREATE INDEX IF NOT EXISTS idx_chat_history_document ON chat_history(document_id);


-- Bắt đầu chạy từ dòng này


-- Update document_metadata table to include user_id
ALTER TABLE document_metadata 
ADD COLUMN IF NOT EXISTS user_id BIGINT REFERENCES users(id) ON DELETE CASCADE;

-- Update documents table to include user_id
ALTER TABLE documents 
ADD COLUMN IF NOT EXISTS user_id BIGINT REFERENCES users(id) ON DELETE CASCADE;

-- Update chat_history to include more metadata
ALTER TABLE chat_history 
ADD COLUMN IF NOT EXISTS user_id BIGINT REFERENCES users(id) ON DELETE CASCADE;

-- Update indexes
CREATE INDEX IF NOT EXISTS idx_document_metadata_user ON document_metadata(user_id);
CREATE INDEX IF NOT EXISTS idx_documents_user ON documents(user_id);
CREATE INDEX IF NOT EXISTS idx_chat_history_user ON chat_history(user_id);

-- Add updated_at column if not exists
ALTER TABLE document_metadata 
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW();

-- Update match_documents function to support user filtering
DROP FUNCTION IF EXISTS match_documents(vector, int, jsonb);

CREATE OR REPLACE FUNCTION match_documents (
  query_embedding vector(768),
  match_count int DEFAULT 5,
  filter jsonb DEFAULT '{}',
  user_filter bigint DEFAULT NULL
) RETURNS TABLE (
  id bigint,
  content text,
  metadata jsonb,
  similarity float
)
LANGUAGE plpgsql
AS $$
#variable_conflict use_column
BEGIN
  RETURN QUERY
  SELECT
    documents.id,
    documents.content,
    documents.metadata,
    1 - (documents.embedding <=> query_embedding) AS similarity
  FROM documents
  WHERE documents.metadata @> filter
    AND (user_filter IS NULL OR documents.user_id = user_filter)
  ORDER BY documents.embedding <=> query_embedding
  LIMIT match_count;
END;
$$;

-- View for admin: User statistics
CREATE OR REPLACE VIEW user_document_stats AS
SELECT 
  u.id as user_id,
  u.email,
  u.full_name,
  u.department,
  COUNT(DISTINCT dm.id) as document_count,
  COUNT(DISTINCT ch.id) as chat_count,
  MAX(ch.created_at) as last_chat_at,
  MAX(dm.created_at) as last_upload_at
FROM users u
LEFT JOIN document_metadata dm ON u.id = dm.user_id
LEFT JOIN chat_history ch ON u.id = ch.user_id
WHERE u.role = 'user'
GROUP BY u.id, u.email, u.full_name, u.department;

-- Assign documents cũ cho admin
UPDATE document_metadata 
SET user_id = (SELECT id FROM users WHERE role = 'admin' LIMIT 1)
WHERE user_id IS NULL;

UPDATE documents 
SET user_id = (SELECT id FROM users WHERE role = 'admin' LIMIT 1)
WHERE user_id IS NULL;

UPDATE chat_history 
SET user_id = (SELECT id FROM users WHERE role = 'admin' LIMIT 1)
WHERE user_id IS NULL;